...
# The ids argument gives custom names to each individual test case,
    # which makes your test output easier to read and debug.
# ids is a built-in keyword argument that pytest.mark.parametrize 
    # recognizes and uses automatically.
@pytest.mark.parametrize(
    "a, b, expected",
    [
        (6, 4, 10.0),           # Test adding two positive integers
        (0, 0, 0),           # Test adding two zeros
        (-5, 5, 0),          # Test adding a negative and a positive integer
        (2.8, 3.7, 6.5),     # Test adding two positive floats
        (-2.4, 3.9, 1.5),    # Test adding a negative float and a positive float
    ],
    ids=[
...
@pytest.mark.parametrize(
    "a, b, expected",
    [
        (15.0, 7, 8),           # Test subtracting a smaller positive integer from a larger one
        (0, 0, 0),           # Test subtracting two zeros
        (-8, -3, -5),        # Test subtracting a negative integer from another negative integer
        (10.5, 2.3, 8.2),    # Test subtracting two positive floats
...
@pytest.mark.parametrize(
    "a, b, expected",
    [
        (13, 3, 39.0),           # Test multiplying two positive integers
        (0, 17, 0),          # Test multiplying zero with a positive integer
        (-2, -7, 14.0),         # Test multiplying two negative integers
